...
void main() {
  ...
  group('AppAvatar', () {
    testWidgets('renders initials when no background image is provided', (tester) async {
    ...
    testWidgets('renders background image and does not show initials when both are provided', (tester) async {
      ...
      expect(find.byType(CircleAvatar), findsOneWidget);
      expect(find.text('XY'), findsNothing); // Should not show initials if image is present
    ...
    testWidgets('renders empty avatar when both image and initials are null', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: AppAvatar(),
