...
void main() {
  ...
  group('AppTheme', () {
    ...
    test('customColorScheme parameter overrides default colors in light() and dark()', () {
      ...
      final lightTheme = AppTheme.light(customColorScheme: customScheme, useGoogleFonts: false);
      ...
      final darkTheme = AppTheme.dark(customColorScheme: customScheme, useGoogleFonts: false);
    ...
    test('useGoogleFonts toggles text theme configurations correctly', () {
      ...
      expect(systemFontsTheme.textTheme.displayLarge?.fontFamily, isNot(contains('Inter')));
    });

    test('sub-themes (buttons, chips, lists, fields) are properly configured', () {
      final theme = AppTheme.light(useGoogleFonts: false);

      // 1. Elevated button theme
      expect(theme.elevatedButtonTheme.style?.backgroundColor?.resolve({}), AppColors.light.primary);
      expect(theme.elevatedButtonTheme.style?.foregroundColor?.resolve({}), AppColors.light.onPrimary);
      expect(theme.elevatedButtonTheme.style?.elevation?.resolve({}), 0);

      // 2. Outlined button theme
      expect(theme.outlinedButtonTheme.style?.foregroundColor?.resolve({}), AppColors.light.primary);
      expect(theme.outlinedButtonTheme.style?.side?.resolve({}), BorderSide(color: AppColors.light.outline));

      // 3. Input Decoration theme
      expect(theme.inputDecorationTheme.filled, isFalse);
      expect(theme.inputDecorationTheme.contentPadding, const EdgeInsets.symmetric(
        horizontal: AppSpacing.md,
        vertical: AppSpacing.sm,
      ));

      // 4. Chip theme
      expect(theme.chipTheme.backgroundColor, AppColors.light.surfaceContainerLow);
      expect(theme.chipTheme.side, BorderSide(color: AppColors.light.outlineVariant));

      // 5. List Tile theme
      expect(theme.listTileTheme.contentPadding, const EdgeInsets.symmetric(horizontal: AppSpacing.md));
      ...
      expect(theme.checkboxTheme.fillColor?.resolve({WidgetState.selected}), AppColors.light.primary);
      expect(theme.checkboxTheme.fillColor?.resolve({}), isNull);

      // 8. Radio theme selected state
      expect(theme.radioTheme.fillColor?.resolve({WidgetState.selected}), AppColors.light.primary);
      expect(theme.radioTheme.fillColor?.resolve({}), isNull);

      // 9. Text and Icon button themes
      expect(theme.textButtonTheme.style?.foregroundColor?.resolve({}), AppColors.light.primary);
      ...
      expect(theme.datePickerTheme.backgroundColor, AppColors.light.surfaceContainerHigh);
      expect(theme.datePickerTheme.weekdayStyle?.fontWeight, FontWeight.bold);
      expect(theme.datePickerTheme.dayForegroundColor?.resolve({WidgetState.selected}), AppColors.light.onPrimary);
      expect(theme.datePickerTheme.dayForegroundColor?.resolve({WidgetState.disabled}), AppColors.light.onSurface.withValues(alpha: 0.38));
      expect(theme.datePickerTheme.dayBackgroundColor?.resolve({WidgetState.selected}), AppColors.light.primary);
      expect(theme.datePickerTheme.dayBackgroundColor?.resolve({}), Colors.transparent);

      // 13. Time Picker theme
      expect(theme.timePickerTheme.backgroundColor, AppColors.light.surfaceContainerHigh);
      expect(theme.timePickerTheme.hourMinuteColor, AppColors.light.surfaceContainerHighest);
      expect(theme.timePickerTheme.hourMinuteTextColor, AppColors.light.onSurface);
