...
/// Interceptor that automatically retries failed requests due to transient errors
/// using exponential backoff with randomized jitter.
class RetryInterceptor extends Interceptor {
  /// Creates a [RetryInterceptor] with the specified [dio] client and [options].
  RetryInterceptor({
    required this.dio,
  ...
  /// The configuration options detailing retry counts, delays, and status codes.
  ...
  Future<void> onError(
    ...
    if (currentAttempt < options.maxRetries && _shouldRetry(err)) {
      ...
      final backoffDelayMs = (options.initialRetryDelay.inMilliseconds *
              pow(options.retryBackoffFactor, currentAttempt))
          .round();

      // Add randomized jitter to prevent thundering herd (e.g. up to 25% random adjustment)
      final jitter = (backoffDelayMs * 0.25 * _random.nextDouble()).round();
      final finalDelayMs = backoffDelayMs + (_random.nextBool() ? jitter : -jitter);
